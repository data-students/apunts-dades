	...
	let filteredNotes = $derived(
		data.notes.filter((note) => {
			...
			if (searchQuery) {
				...
				const authorMatch =
					!note.hideAuthor && note.expand?.author?.name
						? note.expand.author.name.toLowerCase().includes(query)
						: false;

				const courseMatch = note.course ? note.course.toLowerCase().includes(query) : false;
...
<div class="flex flex-col-reverse gap-4 md:flex-row md:items-center md:justify-between">
	...
	<div class="relative w-full md:max-w-sm">
		...
		<Input placeholder="Busca apunts..." class="pl-9 w-full" bind:value={searchQuery} />
	</div>
</div>
